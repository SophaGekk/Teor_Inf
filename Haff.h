...
struct Node_huff {   
    ...
    Node_huff(std::string symbol, double frequency) : symbol(symbol), frequency(frequency), left(nullptr), right(nullptr) {}  
}; 
int main_haff(std::map<std::string, double> frequencies, std::string text, std::vector<std::string> blocks);
#endif 